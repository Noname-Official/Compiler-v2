...
fn gen_maybe(fields: &Fields) -> proc_macro2::TokenStream {
    let first_field_ty = &fields
        .iter()
        .next()
        .map_or(quote! {true}, |field| field.ty.to_token_stream());
...
fn derive_parse_enum(name: Ident, input: DataEnum) -> TokenStream {
    ...
    let first_field_ty = input
        .variants
        .iter()
        .filter_map(|variant| variant.fields.iter().next().map(|field| &field.ty))
        .next()
        .unwrap();
    ...
    let maybe: Punctuated<_, Token![||]> = Punctuated::from_iter(
        input
            .variants
            .iter()
